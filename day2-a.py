...
def mergeMinColors(prev: defaultdict[str, int], curr: defaultdict[str, int]):
    return {
        "red": max(prev["red"], curr["red"]),
        "green": max(prev["green"], curr["green"]),
        "blue": max(prev["blue"], curr["blue"])
    }
...
def parseSet(setStr: str):
    ...
    colorSet = defaultdict(lambda: 0, [parsePair(c) for c in colors])
    return colorSet
...
minColors = [reduce(mergeMinColors, game) for game in games]
powers = [reduce(lambda a,b: a*b, set.values()) for set in minColors]
print(sum(powers))
